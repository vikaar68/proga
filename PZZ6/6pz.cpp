...
int main() {
    try {
        ...
        try {
            arr1.setValue(10, 50); // Несуществующий индекс
        } catch (const std::out_of_range& e) {
            std::cout << "Caught out_of_range: " << e.what() << std::endl;
        }

        try {
            arr1.setValue(0, 150); // Недопустимое значение
        } catch (const std::invalid_argument& e) {
            std::cout << "Caught invalid_argument: " << e.what() << std::endl;
        }

        try {
            arr1.getValue(10); // Несуществующий индекс
        } catch (const std::out_of_range& e) {
            std::cout << "Caught out_of_range: " << e.what() << std::endl;
        }

        try {
            arr1.pushBack(150); // Недопустимое значение
        } catch (const std::invalid_argument& e) {
            std::cout << "Caught invalid_argument: " << e.what() << std::endl;
        }
